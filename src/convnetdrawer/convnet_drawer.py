...
import src.convnetdrawer.config as config
...
class Model:
    ...
    def add_feature_map(self, layer):
        if isinstance(self.feature_maps[-1], FeatureMap3D):
            ...
            elif isinstance(layer, drawFlatten):
...
class drawMaxPooling2D(PoolingLayer):
    def get_description(self):
...
class drawFlatten(Layer):
    def __init__(self):
        super(drawFlatten, self).__init__()
...
class drawDense(Layer):
    def __init__(self, units):
        super(drawDense, self).__init__(filters=units)
...
def main():
    ...
    model.add(drawConv2D(32, (11, 11), (2, 2), padding="same"))
    model.add(drawMaxPooling2D((2, 2)))
    model.add(drawConv2D(64, (7, 7), padding="same"))
    model.add(AveragePooling2D((2, 2)))
    model.add(drawConv2D(128, (3, 3), padding="same"))
    model.add(drawMaxPooling2D((2, 2)))
    model.add(drawConv2D(256, (3, 3), padding="same"))
    model.add(drawConv2D(512, (3, 3), padding="same"))
